    ...
    <div className="activity-list">
      {recordActivities.length > 0 && <>
        <div className="activity-record-icons" aria-label={`${recordActivities.length} completed actions`}>
          {recordActivities.slice(-12).map((activity, index) => <span key={`${activity.id}-icon-${index}`} className={`activity-record-icon activity-record-icon-${activity.status}`} title={activity.title || activity.type}>{iconFor(activity.type, activity.status)}</span>)}
        </div>
