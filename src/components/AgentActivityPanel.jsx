...
export default function AgentActivityPanel({ taskName = "Working on your project", activities = [], startedAt, checkpointAt }) {
  ...
  return <aside className="agent-activity-panel">
    ...
    <div className="activity-summary"><div className="summary-actions">{Object.entries(actionGroups).map(([group, count]) => <span className="action-chip" key={group} title={group}>{group === "files" ? <FileCode2 size={15}/> : group === "tools" ? <Terminal size={15}/> : group === "tests" ? <TestTube2 size={15}/> : group === "preview" ? <Play size={15}/> : group === "agent" ? <Bot size={15}/> : <Zap size={15}/>}<span>{count}</span></span>)}<span className="summary-count">{normalized.length} actions</span></div><div className="summary-item"><Clock3 size={15}/><span>Worked for {formatDuration(elapsed)}</span></div>{checkpointAt && <div className="summary-item checkpoint"><CheckCircle2 size={15}/><span>Checkpoint made {formatTime(new Date(checkpointAt).getTime())}</span></div>}<div className="completion-summary">{completed}/{normalized.length} completed</div></div>
  </aside>;
}
