...
import {
  ...
  ShoppingCart, GraduationCap, Landmark, CalendarDays, MessageCircle, Bot, Store, SlidersHorizontal, Layers3, Boxes, ClipboardList, BarChart3, LockKeyhole, LogOut, Clock3, LayoutTemplate,
} from "lucide-react";
...
const BUILD_LAUNCHER_TYPES = [
  { Icon: Globe, label: "Website", category:"Foundation", description: "Build a complete web application.", prompt: "Build a complete web application" },
  { Icon: Code2, label: "Mobile App", category:"Foundation", description: "Create a responsive mobile application.", prompt: "Create a production-ready mobile application" },
  { Icon: Package, label: "Admin Dashboard", category:"Business", description: "Create an admin or business dashboard.", prompt: "Build an admin dashboard with navigation, metrics, tables, filters, and responsive layouts" },
  { Icon: Server, label: "API Platform", category:"Engineering", description: "Build a backend or API.", prompt: "Build a production-ready API with validation, authentication, error handling, and documentation" },
  { Icon: Sparkles, label: "Landing Page", category:"Marketing", description: "Create a marketing website.", prompt: "Create a polished conversion-focused landing page with responsive sections and clear calls to action" },
  { Icon: ShieldCheck, label: "Authentication", category:"Core", description: "Add secure sign up and login.", prompt: "Build authentication with sign up, login, logout, protected routes, validation, password reset, and session handling" },
  { Icon: ShoppingCart, label: "E-commerce Store", category:"Commerce", description: "Create a store with products and checkout.", prompt: "Build an e-commerce store with product listings, search, filters, cart, checkout flow, order history, and an admin product manager" },
  { Icon: CalendarDays, label: "Booking Platform", category:"Business", description: "Schedule appointments and manage availability.", prompt: "Build a booking platform with calendars, availability rules, appointment creation, confirmation, cancellation, and customer management" },
  { Icon: Store, label: "Marketplace", category:"Commerce", description: "Connect sellers and customers.", prompt: "Build a marketplace with seller profiles, listings, search, filters, favorites, inquiries, and seller management" },
  { Icon: BarChart3, label: "Analytics App", category:"Business", description: "Track metrics and visualize performance.", prompt: "Build an analytics application with KPI cards, charts, date filters, comparisons, data tables, and export actions" },
  { Icon: ClipboardList, label: "Project Manager", category:"Productivity", description: "Organize work, tasks, and teams.", prompt: "Build a project management application with workspaces, projects, tasks, statuses, priorities, assignments, comments, and activity history" },
  { Icon: MessageCircle, label: "Customer Support", category:"Operations", description: "Manage conversations and support tickets.", prompt: "Build a customer support platform with inbox, ticket statuses, assignment, internal notes, search, filters, and customer profiles" },
  { Icon: GraduationCap, label: "Learning Platform", category:"Education", description: "Create courses, lessons, and learner progress.", prompt: "Build a learning platform with courses, lessons, enrollment, progress tracking, quizzes, certificates, and instructor management" },
...
const AI_PROVIDER_CARDS = [
  { id:"cloud", Icon:Zap, title:"Cloud AI / Groq", subtitle:"HTTPS Agent Server", description:"Use the default Groq coding agent through Firebox’s HTTPS Agent Contract.", color:"#38BDF8", action:"Select", enabled:true },
  { id:"local", Icon:Server, title:"Local AI / Ollama", subtitle:"Your local model", description:"Use Ollama through the server-side local AI connection.", color:"#4EC994", action:"Configure", enabled:true },
  { id:"groq-agent", Icon:Server, title:"Groq Agent", subtitle:"HTTPS Agent Server", description:"Use the independent Groq coding agent through the HTTPS Agent Contract.", color:"#F97316", action:"Select", enabled:true },
  { id:"openrouter-agent", Icon:GitBranch, title:"OpenRouter Agent", subtitle:"HTTPS Agent Server", description:"Use the independent OpenRouter coding agent through the HTTPS Agent Contract.", color:"#C084FC", action:"Select", enabled:true },
  { id:"openai-agent", Icon:Brain, title:"OpenAI Agent", subtitle:"HTTPS Agent Server", description:"Use the independent OpenAI coding agent through the HTTPS Agent Contract.", color:"#4EC994", action:"Select", enabled:true },
  { id:"claude-agent", Icon:Sparkles, title:"Claude Agent", subtitle:"HTTPS Agent Server", description:"Use the independent Claude coding agent through the HTTPS Agent Contract.", color:"#F3A65B", action:"Select", enabled:true },
  { id:"gemini-agent", Icon:Globe, title:"Gemini Agent", subtitle:"HTTPS Agent Server", description:"Use the independent Gemini coding agent through the HTTPS Agent Contract.", color:"#60A5FA", action:"Select", enabled:true },
  { id:"firebox-agent", Icon:Flame, title:"Firebox Agent", subtitle:"External HTTPS Agent Server", description:"Use the separate configurable Firebox coding agent backed by vLLM and Qwen.", color:"#F97316", action:"Select", enabled:true },
  { id:"custom", Icon:Settings, title:"Custom OpenAI-compatible", subtitle:"Your endpoint", description:"Use a custom endpoint only when you intentionally configure it.", color:"#94A3B8", action:"Configure", enabled:true },
];
...
export default function FireboxAIStudio() {
  ...
  const copyPreviewLink = useCallback(async () => {
    ...
    try { await navigator.clipboard.writeText(previewUrl); setPreviewLinkCopied(true); window.setTimeout(() => setPreviewLinkCopied(false), 1600); } catch {}
  }, [previewUrl]);
    ...
    try {
      const res = await fetch("/api/build", {
        ...
        body: JSON.stringify({
          ...
          stackPreferences: { framework: launcherFramework, packageManager: launcherPackageManager, database: launcherDatabase },
          toolMode: true,
        }),
      });
  ...
  const requestBuildPlan = useCallback(async (requestText) => {
    ...
    setBuildPlan(null);
    try {
      if (["cloud", "groq-agent", "openrouter-agent", "openai-agent", "gemini-agent", "claude-agent", "firebox-agent"].includes(aiProvider)) {
      ...
      let response;
      try {
        response = await fetch(requestUrl, { method:"POST", headers, body: JSON.stringify({ description:text, fileNames:allFiles.map(file => file.path), provider:aiProvider, localAi:aiProvider !== "cloud" ? providerConfig : undefined, stackPreferences: { framework: launcherFramework, packageManager: launcherPackageManager, database: launcherDatabase } }) });
      } catch (networkError) {
...
{isActive
              ...
              {activity === "plugins" && (
                <div style={{ flex:1, minHeight:0, overflowY:"auto", background:palette.editorBg, color:palette.text, fontFamily:FONT_UI }}>
                  <div style={{ maxWidth:"none", margin:0, padding:isMobile ? "24px 16px 42px" : "34px 18px 56px" }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:isMobile ? "1fr" : "repeat(3, minmax(0, 1fr))", gap:12 }}>
                      {FIREBOX_PLUGIN_CATALOG.map(({Icon,name,description,capabilities}) => <div key={name} onClick={() => { setChatInput(`Use the ${name} plugin to configure and verify this project: ${description}`); setActivity("workspace"); setSideOpen(false); setTimeout(() => chatInputRef.current?.focus(), 80); }} title={`Use ${name} with the Agent`} style={{ minHeight:168, cursor:"pointer", display:"flex", flexDirection:"column", padding:17, border:`1px solid ${palette.border}`, background:palette.panelBg }}><div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}><div style={{ width:34, height:34, display:"flex", alignItems:"center", justifyContent:"center", background:`${palette.accent}18`, color:palette.accent }}><Icon size={18}/></div><span style={{ color:palette.success, fontSize:9, fontWeight:700, letterSpacing:"0.08em" }}>AVAILABLE</span></div><div style={{ marginTop:15, color:palette.textActive, fontSize:14, fontWeight:700 }}>{name}</div><div style={{ flex:1, marginTop:7, color:palette.textMuted, fontSize:11, lineHeight:1.55 }}>{description}</div><div style={{ marginTop:13, paddingTop:10, borderTop:`1px solid ${palette.border}`, color:palette.textFaint, fontSize:10, fontFamily:FONT_MONO }}>{capabilities}</div></div>)}
                    </div>
                  </div>
                </div>
              )}
            ...
            const editorContent = activity === "settings" ? settingsPage : activity === "home" ? (
              ...
              <div style={{ flex:1, minHeight:0, overflowY:"auto", background:palette.editorBg, color:palette.text, fontFamily:FONT_UI }}><div style={{ maxWidth:"none", margin:0, padding:isMobile ? "24px 16px 42px" : "34px 18px 56px" }}><div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:18, marginBottom:24 }}><div><div style={{ display:"flex", alignItems:"center", gap:8, color:palette.accent, fontSize:11, fontWeight:700, letterSpacing:"0.12em", marginBottom:8 }}><Boxes size={14}/> FIREBOX PLUGINS</div><div style={{ color:palette.textActive, fontSize:isMobile ? 24 : 30, fontWeight:750 }}>Extend your Agent</div><div style={{ color:palette.textMuted, fontSize:12, lineHeight:1.6, marginTop:8, maxWidth:650 }}>Give Firebox focused capabilities for databases, browser testing, previews, deployment, code quality, and integrations.</div></div><div style={{ display:isMobile ? "none" : "flex", alignItems:"center", gap:7, padding:"8px 10px", border:`1px solid ${palette.border}`, color:palette.textMuted, fontSize:10 }}><span style={{ width:7, height:7, borderRadius:"50%", background:palette.success }}/>{FIREBOX_PLUGIN_CATALOG.length} capabilities available</div></div><div style={{ display:"grid", gridTemplateColumns:isMobile ? "1fr" : "repeat(3, minmax(0, 1fr))", gap:12 }}>{FIREBOX_PLUGIN_CATALOG.map(({Icon,name,description,capabilities}) => <div key={name} onClick={() => { setChatInput(`Use the ${name} plugin to configure and verify this project: ${description}`); setActivity("workspace"); setSideOpen(false); setTimeout(() => chatInputRef.current?.focus(), 80); }} title={`Use ${name} with the Agent`} style={{ minHeight:168, cursor:"pointer", display:"flex", flexDirection:"column", padding:17, border:`1px solid ${palette.border}`, background:palette.panelBg }}><div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}><div style={{ width:34, height:34, display:"flex", alignItems:"center", justifyContent:"center", background:`${palette.accent}18`, color:palette.accent }}><Icon size={18}/></div><span style={{ color:palette.success, fontSize:9, fontWeight:700 }}>AVAILABLE</span></div><div style={{ marginTop:15, color:palette.textActive, fontSize:14, fontWeight:700 }}>{name}</div><div style={{ flex:1, marginTop:7, color:palette.textMuted, fontSize:11, lineHeight:1.55 }}>{description}</div><div style={{ marginTop:13, paddingTop:10, borderTop:`1px solid ${palette.border}`, color:palette.textFaint, fontSize:10, fontFamily:FONT_MONO }}>{capabilities}</div></div>)}</div></div></div>
            ) : activity === "git" ? (
              <div style={{ flex:1, minHeight:0, overflowY:"auto", background:palette.editorBg, color:palette.text, fontFamily:FONT_UI }}><div style={{ maxWidth:760, margin:"0 auto", padding:isMobile ? "24px 16px 42px" : "42px 34px" }}><div style={{ display:"flex", alignItems:"center", gap:10, color:palette.textActive, fontSize:22, fontWeight:750 }}><GitBranch size={21} color={palette.accent}/>Source Control</div><div style={{ color:palette.textMuted, fontSize:12, lineHeight:1.6, marginTop:8, marginBottom:24 }}>Connect GitHub with OAuth or a Personal Access Token to browse repositories and manage project changes.</div>{!gitTokenSaved ? <div style={{ padding:18, border:`1px solid ${palette.border}`, background:palette.panelBg }}><div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:5, marginBottom:14, padding:3, background:palette.editorBg, border:`1px solid ${palette.border}` }}><button onClick={() => setGitAuthMethod("oauth")} style={{ padding:"9px 6px", border:"none", background:gitAuthMethod === "oauth" ? palette.accent : "transparent", color:gitAuthMethod === "oauth" ? "#fff" : palette.textMuted, cursor:"pointer", fontSize:11, fontWeight:700 }}>GitHub OAuth</button><button onClick={() => setGitAuthMethod("pat")} style={{ padding:"9px 6px", border:"none", background:gitAuthMethod === "pat" ? palette.accent : "transparent", color:gitAuthMethod === "pat" ? "#fff" : palette.textMuted, cursor:"pointer", fontSize:11, fontWeight:700 }}>Personal Access Token</button></div>{gitAuthMethod === "oauth" ? <><div style={{ color:palette.textMuted, fontSize:12, lineHeight:1.6, marginBottom:14 }}>Authorize Firebox through GitHub. No token is pasted into the application.</div><button onClick={startGithubOAuth} style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"10px 15px", border:"none", background:palette.accent, color:"#fff", cursor:"pointer", fontWeight:700, fontSize:12 }}><Github size={14}/> Continue with GitHub</button></> : <><div style={{ color:palette.textMuted, fontSize:12, lineHeight:1.6, marginBottom:10 }}>Paste a GitHub token with repository access. Firebox validates it before saving.</div><div style={{ display:"flex", gap:8 }}><input type="password" value={gitTokenInput} onChange={e => setGitTokenInput(e.target.value)} placeholder="ghp_…" style={{ flex:1, minWidth:0, padding:"10px 11px", border:`1px solid ${palette.borderLight}`, background:palette.editorBg, color:palette.textActive, fontFamily:FONT_MONO, outline:"none" }}/><button onClick={saveGitToken} disabled={gitTokenSaving || !gitTokenInput.trim()} style={{ padding:"10px 15px", border:"none", background:gitTokenInput.trim() ? palette.accent : palette.border, color:"#fff", cursor:gitTokenInput.trim() ? "pointer" : "not-allowed", fontWeight:700 }}>Connect</button></div></>}{gitError && <div style={{ marginTop:12, color:palette.error, fontSize:11 }}>{gitError}</div>}</div> : <div style={{ padding:18, border:`1px solid ${palette.border}`, background:palette.panelBg }}><div style={{ color:palette.success, fontSize:13, fontWeight:700 }}>GitHub connected{gitUsername ? ` as ${gitUsername}` : ""}</div><div style={{ color:palette.textMuted, fontSize:11, marginTop:5 }}>Authentication: {gitAuthMethod === "oauth" ? "OAuth" : "Personal Access Token"}</div><div style={{ display:"flex", gap:8, marginTop:16 }}><button onClick={removeGitToken} style={{ padding:"8px 12px", border:`1px solid ${palette.error}66`, background:"transparent", color:palette.error, cursor:"pointer", fontSize:11 }}>Disconnect</button><button onClick={async () => { setGitReposLoading(true); const r = await fetch("/api/git/repos"); const d = await r.json().catch(() => []); if (r.ok) setGitRepos(d); setGitReposLoading(false); }} style={{ padding:"8px 12px", border:`1px solid ${palette.border}`, background:"transparent", color:palette.text, cursor:"pointer", fontSize:11 }}>Refresh repositories</button></div>{gitReposLoading ? <div style={{ marginTop:16, color:palette.textMuted, fontSize:11 }}>Loading repositories…</div> : gitRepos.length > 0 && <div style={{ marginTop:16, display:"grid", gap:6 }}>{gitRepos.map(repo => <div key={repo.id} style={{ padding:"9px 10px", border:`1px solid ${palette.border}`, color:palette.text, fontSize:11 }}>{repo.fullName}<span style={{ float:"right", color:palette.textFaint }}>{repo.private ? "Private" : "Public"}</span></div>)}</div>}</div>}</div></div>
            ) : activity === "projects" ? (
              <div style={{ flex:1, minHeight:0, overflowY:"auto", background:palette.editorBg, color:palette.text, fontFamily:FONT_UI }}><div style={{ padding:isMobile ? "24px 16px 42px" : "36px 28px 56px" }}><div style={{ color:palette.textActive, fontSize:24, fontWeight:750 }}>My Projects</div><div style={{ color:palette.textMuted, fontSize:12, marginTop:7, marginBottom:26 }}>Open a project or create a new one.</div><div style={{ display:"grid", gridTemplateColumns:isMobile ? "1fr" : "repeat(auto-fill, minmax(220px, 1fr))", gap:14 }}>{recentBuilds.map(build => <div key={build._id} onClick={() => loadProjectFiles(build)} style={{ padding:17, border:`1px solid ${palette.border}`, background:palette.panelBg, cursor:"pointer" }}><div style={{ color:palette.textActive, fontSize:14, fontWeight:700 }}>{build.description || "Untitled project"}</div><div style={{ display:"flex", gap:9, marginTop:10, color:build.status === "complete" ? palette.success : palette.textMuted, fontSize:11 }}><span>{build.status}</span><span>{build.files?.length || 0} files</span></div></div>)}<button onClick={() => { setActivity("workspace"); setSideOpen(false); setTimeout(() => chatInputRef.current?.focus(), 80); }} style={{ minHeight:120, border:`1px dashed ${palette.borderLight}`, background:"transparent", color:palette.textMuted, cursor:"pointer", fontSize:12 }}>+ New project</button></div></div></div>
            ) : activity === "secrets" ? (
              <div style={{ flex:1, minHeight:0, overflowY:"auto", background:palette.editorBg, color:palette.text, fontFamily:FONT_UI }}>
              ...
              <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", position:"relative", background:palette.editorBg, color:palette.text, fontFamily:FONT_UI }}>
                {phase === "building" && <div style={{ height:34, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"flex-end", padding:"0 14px", borderBottom:`1px solid ${palette.border}`, background:palette.titleBar }}><div style={{ display:"flex", alignItems:"center", gap:8, color:palette.textMuted, fontSize:11 }}><span style={{ width:7, height:7, borderRadius:"50%", background:palette.accent }}/><span>Agents working</span><button onClick={() => setBuildExecutionState(buildPaused ? "running" : "paused")} style={{ marginLeft:6, border:`1px solid ${palette.accent}66`, borderRadius:6, background:`${palette.accent}12`, color:palette.accent, padding:"4px 8px", fontSize:10, cursor:"pointer" }}>{buildPaused ? "Resume" : "Pause"}</button><button onClick={stopBuild} style={{ marginLeft:4, border:`1px solid ${palette.error}66`, borderRadius:6, background:`${palette.error}12`, color:palette.error, padding:"4px 8px", fontSize:10, cursor:"pointer" }}>Stop Agent</button></div></div>}
                <div style={{ height:40, flexShrink:0, display:"flex", alignItems:"center", gap:4, padding:"0 14px", borderBottom:`1px solid ${palette.border}`, background:palette.tabBar }}>
                  {[{id:"code", label:"Code", Icon:Code2}, {id:"preview", label:"Live Preview", Icon:Eye}, {id:"templates", label:"Templates", Icon:LayoutTemplate}].map(({id,label,Icon}) => <button key={id} onClick={() => { setWorkspaceMode(id); if (id === "preview") setPreviewOpen(true); }} style={{ display:"inline-flex", alignItems:"center", gap:6, height:28, padding:"0 10px", border:"none", borderBottom:`2px solid ${workspaceMode === id ? palette.accent : "transparent"}`, background:"transparent", color:workspaceMode === id ? palette.textActive : palette.textMuted, cursor:"pointer", fontFamily:FONT_UI, fontSize:11, fontWeight:workspaceMode === id ? 700 : 500 }}><Icon size={13}/>{label}</button>)}
                </div>
                ...
                {workspaceMode === "templates" ? <div style={{ flex:1, overflowY:"auto", padding:isMobile ? 18 : 30, background:palette.editorBg }}><div style={{ color:palette.textActive, fontSize:20, fontWeight:700 }}>Project templates</div><div style={{ color:palette.textMuted, fontSize:12, marginTop:6, marginBottom:22 }}>Start a new project direction without leaving the workspace.</div><div style={{ display:"grid", gridTemplateColumns:isMobile ? "1fr" : "repeat(3, minmax(0, 1fr))", gap:14 }}>{BUILD_LAUNCHER_TYPES.map(({Icon,label,category,description,prompt}) => <button key={label} onClick={() => { setWorkspaceMode("code"); setChatInput(prompt); setTimeout(() => chatInputRef.current?.focus(), 80); }} style={{ minHeight:150, padding:18, textAlign:"left", border:`1px solid ${palette.border}`, background:palette.panelBg, color:palette.text, cursor:"pointer" }}><Icon size={22} color={palette.accent}/><div style={{ marginTop:16, color:palette.textFaint, fontSize:9, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase" }}>{category}</div><div style={{ marginTop:5, color:palette.textActive, fontSize:14, fontWeight:700 }}>{label}</div><div style={{ marginTop:7, color:palette.textMuted, fontSize:11, lineHeight:1.5 }}>{description}</div></button>)}</div></div> : workspaceMode === "preview" ? <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", background:palette.editorBg }}><div style={{ height:42, flexShrink:0, display:"flex", alignItems:"center", gap:8, padding:"0 12px", borderBottom:`1px solid ${palette.border}`, background:palette.titleBar }}><Globe size={14} color={previewStatus === "running" ? palette.success : palette.textMuted}/><input value={previewUrl || ""} readOnly placeholder="Development link will appear when preview is running" style={{ flex:1, minWidth:0, border:"none", outline:"none", background:"transparent", color:palette.textMuted, fontFamily:FONT_MONO, fontSize:11 }}/><span style={{ color:previewStatus === "running" ? palette.success : palette.textMuted, fontSize:10 }}>{previewStatus === "running" ? "Live" : previewStatus === "starting" ? "Starting" : "Offline"}</span><button onClick={copyPreviewLink} disabled={!previewUrl} title="Copy development link" style={{ border:"none", background:"transparent", color:previewUrl ? palette.text : palette.textFaint, cursor:previewUrl ? "pointer" : "not-allowed", fontSize:11 }}>{previewLinkCopied ? "Copied" : "Copy link"}</button><button onClick={refreshPreview} disabled={!previewUrl} title="Refresh browser view" style={{ border:"none", background:"transparent", color:previewUrl ? palette.text : palette.textFaint, cursor:previewUrl ? "pointer" : "not-allowed" }}><RefreshCw size={13}/></button></div><div style={{ flex:1, minHeight:0, background:"#111" }}>{previewUrl ? <iframe key={`${previewUrl}-${previewRefreshKey}`} title="Firebox in-app browser view" src={previewUrl} style={{ width:"100%", height:"100%", border:"none", background:"#fff" }} /> : <div style={{ height:"100%", display:"flex", alignItems:"center", justifyContent:"center", color:palette.textMuted, fontSize:12 }}>{previewStatus === "starting" ? "Starting live preview…" : "Start the project preview to view the running application here."}</div>}</div></div> : <PanelGroup orientation={isMobile ? "vertical" : "horizontal"} style={{ flex:1, minHeight:0, overflow:"hidden" }}>
                  <Panel defaultSize={isMobile ? "42%" : "34%"} minSize={isMobile ? "28%" : "20%"} maxSize={isMobile ? "65%" : "50%"} style={{ minHeight:0, display:"flex", flexDirection:"column", borderRight:isMobile ? "none" : `1px solid ${palette.border}`, background:palette.sideBar, overflow:"hidden" }}>
                    ...
                    <div style={{ flexShrink:0, padding:"10px 10px 12px", borderTop:`1px solid ${palette.border}`, background:palette.titleBar }}>
                      <div style={{ border:`1px solid ${palette.borderLight}`, borderRadius:16, background:palette.editorBg, padding:"10px 10px 8px", boxShadow:"0 8px 24px rgba(0,0,0,0.16)" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:7 }}>
                          ...
                          <button onClick={sendChatMessage} disabled={!chatInput.trim() || phase === "building" || aiThinking} aria-label="Send prompt" title="Send" style={{ width:30, height:30, border:"none", borderRadius:"50%", background:chatInput.trim() ? palette.accent : palette.border, color:chatInput.trim() ? "#fff" : palette.textFaint, display:"flex", alignItems:"center", justifyContent:"center", cursor:chatInput.trim() ? "pointer" : "not-allowed" }}><Send size={13}/></button>
                        </div>
                      </div>
                    </div>
                  </Panel>
                  <PanelResizeHandle style={{ width:isMobile ? "100%" : 4, height:isMobile ? 4 : "100%", background:palette.border, cursor:isMobile ? "row-resize" : "col-resize", flexShrink:0 }} />
                  <Panel minSize={isMobile ? "35%" : "35%"} style={{ minWidth:0, minHeight:0, display:"flex", flexDirection:"column", background:palette.editorBg, overflow:"hidden" }}>
        ...
        {customAiModalOpen && (
          <div role="dialog" aria-modal="true" aria-labelledby="custom-ai-title" onClick={() => setCustomAiModalOpen(false)} style={{ position:"fixed", inset:0, zIndex:520, background:"rgba(0,0,0,0.62)", display:"flex", alignItems:"center", justifyContent:"center", padding:isMobile ? 14 : 24 }}>
            <div onClick={e => e.stopPropagation()} style={{ width:"min(620px, 100%)", maxHeight:"calc(100vh - 28px)", overflowY:"auto", background:palette.sideBar, border:`1px solid ${palette.border}`, borderRadius:14, boxShadow:"0 24px 90px rgba(0,0,0,0.5)", color:palette.text, fontFamily:FONT_UI }}>
              <div style={{ display:"flex", alignItems:"center", gap:12, padding:"18px 20px", borderBottom:`1px solid ${palette.border}` }}>
                <div style={{ width:40, height:40, borderRadius:11, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(167,139,250,0.16)", color:"#A78BFA" }}><Settings size={20}/></div>
                <div style={{ flex:1, minWidth:0 }}><div id="custom-ai-title" style={{ color:palette.textActive, fontSize:16, fontWeight:750 }}>Custom AI Provider</div><div style={{ marginTop:4, color:palette.textMuted, fontSize:11 }}>Connect any OpenAI-compatible chat-completions model to the Firebox Agent.</div></div>
                <button onClick={() => setCustomAiModalOpen(false)} aria-label="Close custom provider settings" style={{ display:"flex", alignItems:"center", justifyContent:"center", width:30, height:30, border:`1px solid ${palette.border}`, borderRadius:7, background:"transparent", color:palette.textMuted, cursor:"pointer" }}><X size={15}/></button>
              </div>
              <div style={{ padding:20, display:"grid", gap:14 }}>
                <div style={{ padding:12, borderRadius:9, border:`1px solid ${palette.accent}44`, background:`${palette.accent}0d`, color:palette.textMuted, fontSize:11, lineHeight:1.55 }}><strong style={{ color:palette.text }}>Firebox keeps control of execution.</strong> Your model provides reasoning and tool calls; Firebox performs file edits, commands, tests, repairs, and preview actions through its own tools.</div>
                <label style={{ display:"grid", gap:6, color:palette.textMuted, fontSize:10, fontWeight:750, letterSpacing:"0.04em" }}>OPENAI-COMPATIBLE ENDPOINT<input autoFocus value={localAiEndpoint} onChange={e => { setLocalAiEndpoint(e.target.value); setLocalAiTestState("idle"); }} placeholder="https://your-provider.example.com/v1" style={{ width:"100%", boxSizing:"border-box", padding:"11px 12px", border:`1px solid ${palette.border}`, borderRadius:8, outline:"none", background:palette.editorBg, color:palette.text, fontFamily:FONT_MONO, fontSize:12 }}/></label>
                <label style={{ display:"grid", gap:6, color:palette.textMuted, fontSize:10, fontWeight:750, letterSpacing:"0.04em" }}>MODEL IDENTIFIER<input value={localAiModel} onChange={e => { setLocalAiModel(e.target.value); setLocalAiTestState("idle"); }} placeholder="your-model-name" style={{ width:"100%", boxSizing:"border-box", padding:"11px 12px", border:`1px solid ${palette.border}`, borderRadius:8, outline:"none", background:palette.editorBg, color:palette.text, fontFamily:FONT_MONO, fontSize:12 }}/></label>
                <label style={{ display:"grid", gap:6, color:palette.textMuted, fontSize:10, fontWeight:750, letterSpacing:"0.04em" }}>API KEY <span style={{ fontWeight:500, letterSpacing:0, textTransform:"none" }}>(optional only if your provider does not require one)</span><input type="password" value={localAiApiKey} onChange={e => { setLocalAiApiKey(e.target.value); setLocalAiTestState("idle"); }} placeholder="sk-…" style={{ width:"100%", boxSizing:"border-box", padding:"11px 12px", border:`1px solid ${palette.border}`, borderRadius:8, outline:"none", background:palette.editorBg, color:palette.text, fontFamily:FONT_MONO, fontSize:12 }}/></label>
                <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}><button onClick={testLocalAi} disabled={localAiTestState === "testing"} style={{ padding:"10px 14px", border:`1px solid ${palette.border}`, borderRadius:8, background:palette.editorBg, color:palette.text, cursor:localAiTestState === "testing" ? "wait" : "pointer", fontFamily:FONT_UI, fontSize:11, fontWeight:700 }}>{localAiTestState === "testing" ? "Testing connection…" : "Test connection"}</button>{localAiTestState !== "idle" && <span style={{ color:localAiTestState === "success" ? palette.success : palette.error, fontSize:11 }}>{localAiTestState === "success" ? "Connection works: " : "Connection failed: "}{localAiTestMessage}</span>}</div>
                <div style={{ padding:12, borderRadius:9, background:palette.editorBg, color:palette.textFaint, fontSize:10, lineHeight:1.55 }}>Endpoint format: <code style={{ color:palette.textMuted, fontFamily:FONT_MONO }}>https://provider.example.com/v1</code>. Firebox appends <code style={{ color:palette.textMuted, fontFamily:FONT_MONO }}>/chat/completions</code> automatically. The values are saved locally in this browser and are not written to GitHub.</div>
              </div>
              <div style={{ display:"flex", justifyContent:"flex-end", gap:9, padding:"14px 20px", borderTop:`1px solid ${palette.border}` }}><button onClick={() => setCustomAiModalOpen(false)} style={{ padding:"9px 13px", border:`1px solid ${palette.border}`, borderRadius:8, background:"transparent", color:palette.textMuted, cursor:"pointer", fontFamily:FONT_UI, fontSize:11, fontWeight:650 }}>Cancel</button><button onClick={() => { setAiProvider("custom"); setCustomAiModalOpen(false); setLocalAiTestState("idle"); }} style={{ padding:"9px 15px", border:"none", borderRadius:8, background:palette.accent, color:"#fff", cursor:"pointer", fontFamily:FONT_UI, fontSize:11, fontWeight:750 }}>Save and use Custom AI</button></div>
            </div>
          </div>
        )}
        {/* ══ Status bar ══════════════════════════════════════════════════ */}
