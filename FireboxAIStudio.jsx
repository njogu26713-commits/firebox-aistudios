          ...
          {(() => {
            ...
            const editorContent = activity === "home" ? (
              ...
              <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", background:"#181818", color:VS.text, fontFamily:FONT_UI }}>
                ...
                {projectOpenStatus?.phase === "opening" && <div style={{ flexShrink:0, padding:"16px 20px", borderBottom:`1px solid ${VS.border}`, background:"linear-gradient(90deg, rgba(0,120,212,0.12), rgba(167,139,250,0.08))" }}><div style={{ display:"flex", alignItems:"center", gap:9, color:VS.textActive, fontSize:13, fontWeight:700 }}><Loader2 size={15} color={VS.accent} style={{ animation:"spin 1s linear infinite" }}/><span>🔥 {projectOpenStatus.message}</span><span style={{ marginLeft:"auto", color:VS.textMuted, fontSize:10 }}>Step {(projectOpenStatus.currentStepIndex || 0) + 1} of {projectOpenStatus.steps.length}</span></div><div style={{ display:"grid", gridTemplateColumns:`repeat(${projectOpenStatus.steps.length}, minmax(0, 1fr))`, gap:8, marginTop:12 }}>{projectOpenStatus.steps.map((step, index) => <div key={`${step}-${index}`} style={{ color:index < projectOpenStatus.currentStepIndex ? VS.success : index === projectOpenStatus.currentStepIndex ? VS.textActive : VS.textFaint, fontSize:10, lineHeight:1.35, display:"flex", alignItems:"center", gap:5 }}><span style={{ width:15, textAlign:"center" }}>{index < projectOpenStatus.currentStepIndex ? "✓" : index === projectOpenStatus.currentStepIndex ? <Loader2 size={11} color={VS.accent} style={{ animation:"spin 1s linear infinite" }}/> : "○"}</span><span>{step}</span></div>)}</div></div>}
