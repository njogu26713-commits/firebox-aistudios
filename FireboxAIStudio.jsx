          ...
          {(() => {
            ...
            const editorContent = activity === "home" ? (
              ...
              <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", position:"relative", background:"#181818", color:VS.text, fontFamily:FONT_UI }}>
                ...
                {projectOpenStatus?.phase === "opening" && <div style={{ position:"absolute", inset:0, zIndex:20, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(24,24,24,0.96)", backdropFilter:"blur(4px)" }}><div style={{ width:"min(420px, calc(100% - 48px))", textAlign:"center", padding:"36px 28px", border:`1px solid ${VS.borderLight}`, borderRadius:16, background:"linear-gradient(145deg, #252526, #1b1b1b)", boxShadow:"0 18px 60px rgba(0,0,0,0.45)" }}><div style={{ width:64, height:64, margin:"0 auto 20px", display:"flex", alignItems:"center", justifyContent:"center", borderRadius:18, background:`${VS.accent}18`, border:`1px solid ${VS.accent}55` }}><Loader2 size={32} color={VS.accent} style={{ animation:"spin 1s linear infinite" }}/></div><div style={{ color:VS.textActive, fontSize:17, fontWeight:700 }}>Opening project</div><div style={{ color:VS.textMuted, fontSize:12, marginTop:8 }}>{projectOpenStatus.message}</div><div style={{ height:4, marginTop:24, overflow:"hidden", borderRadius:999, background:"rgba(255,255,255,0.08)" }}><div style={{ width:`${Math.max(8, (((projectOpenStatus.currentStepIndex || 0) + 1) / projectOpenStatus.steps.length) * 100)}%`, height:"100%", borderRadius:999, background:`linear-gradient(90deg, ${VS.accent}, #A78BFA)`, transition:"width 0.35s ease" }}/></div><div style={{ color:VS.textFaint, fontSize:10, marginTop:10 }}>Step {(projectOpenStatus.currentStepIndex || 0) + 1} of {projectOpenStatus.steps.length}</div></div></div>}
