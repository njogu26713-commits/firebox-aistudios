...
export default function FireboxAIStudio() {
  ...
  const loadProjectFiles = useCallback(async (build) => {
    ...
    setProjectOpenStatus({ phase: "opening", message: "Opening Firebox project…", steps: ["Loading filesystem"] });
    try {
      const res  = await fetch(`/api/build/${build._id}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Unable to load project");
      setProjectOpenStatus({ phase: "opening", message: "Scanning project files…", steps: ["Project loaded"] });
      ...
      setProjectOpenStatus({ phase: "opening", message: "Detecting project setup…", steps: ["Project loaded", `${files.length} files found`, framework ? `${framework} detected` : "Project structure detected", packageManager ? `${packageManager} dependencies detected` : "Project metadata detected"] });
      setAllFiles(files);
      ...
      setProjectOpenStatus({ phase: "ready", message: "Workspace ready", steps: ["Project loaded", `${files.length} files found`, framework ? `${framework} detected` : "Project structure detected", packageManager ? `${packageManager} dependencies detected` : "Project metadata detected", "Workspace ready"] });
    } catch (err) {
      console.error("Project opening failed", err);
      setProjectOpenStatus({ phase: "error", message: err.message || "Unable to open project", steps: [] });
    }
