              ...
              <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", background:"#181818", color:VS.text, fontFamily:FONT_UI }}>
                ...
                <div style={{ flex:1, minHeight:0, display:"grid", gridTemplateColumns:isMobile ? "1fr" : "minmax(250px, 0.34fr) minmax(0, 0.66fr)", gap:0 }}>
                  <div style={{ minHeight:0, display:"flex", flexDirection:"column", borderRight:isMobile ? "none" : `1px solid ${VS.border}`, background:"#252526" }}>
                    <div style={{ padding:"12px 12px 7px", color:VS.textMuted, fontSize:10, fontWeight:800, letterSpacing:"0.1em" }}>AGENT ACTIVITY</div>
                    <div style={{ flex:"0 1 52%", minHeight:170, overflowY:"auto", padding:"0 8px 10px", borderBottom:`1px solid ${VS.border}` }}>
                      <div style={{ display:"flex", justifyContent:"space-between", color:VS.textMuted, fontSize:10, marginBottom:6 }}><span>Pipeline</span><span>{doneCount}/{AGENT_META.length} · {Math.round(progress)}%</span></div>
                      <div style={{ height:3, background:"rgba(255,255,255,0.08)", borderRadius:3, marginBottom:8 }}><div style={{ height:"100%", width:`${progress}%`, background:phase === "complete" ? VS.success : VS.accent, borderRadius:3 }}/></div>
                      {AGENT_META.map(({ name:agentName, Icon, color }) => { const state = agentStates.find(a => a.name === agentName) || { status:"idle", streaming:"" }; const status = state.status || "idle"; const active = activeAgent === agentName || status === "running" || status === "working"; const label = active ? "Working now" : status === "done" ? "Completed" : status === "error" ? "Needs attention" : "Waiting"; const steps = AGENT_STEPS[agentName] || []; const stepIndex = Math.max(0, Math.min((agentVisSteps[agentName] || 1) - 1, steps.length - 1)); const detail = state.streaming || (active || status === "done" ? steps[stepIndex]?.text : "Waiting for the previous agent"); return <div key={agentName} style={{ padding:"7px 7px", borderRadius:7, background:active ? `${color}12` : "transparent", borderLeft:`2px solid ${active ? color : "transparent"}` }}><div style={{ display:"flex", alignItems:"center", gap:7 }}><Icon size={13} color={color}/><span style={{ color:VS.textActive, fontSize:11, fontWeight:active || status === "done" ? 650 : 500, flex:1 }}>{agentName}</span><span style={{ color:active ? color : status === "done" ? VS.success : status === "error" ? VS.error : VS.textFaint, fontSize:9 }}>{label}</span></div><div style={{ marginLeft:20, marginTop:3, color:VS.textFaint, fontSize:9, lineHeight:1.35, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{detail || "Ready for work"}</div></div>; })}
                    </div>
                    <div style={{ padding:"10px 12px 6px", color:VS.textMuted, fontSize:10, fontWeight:800, letterSpacing:"0.1em" }}>EXPLORER</div>
                    <div style={{ flex:"1 1 48%", minHeight:90, overflowY:"auto", padding:"0 8px 10px" }}><div style={{ display:"flex", alignItems:"center", gap:6, height:25, color:VS.text, fontSize:11, fontWeight:700 }}><ChevronDown size={12} color={VS.textMuted}/><Zap size={12} color={VS.accent}/><span>firebox-project</span></div>{allFiles.length === 0 ? <div style={{ padding:"8px 18px", color:VS.textFaint, fontSize:10 }}>Files created by the agents will appear here.</div> : AGENT_META.map(({ name:agentName, Icon, color }) => { const agentFiles = filesByAgent[agentName] || []; if (!agentFiles.length) return null; const groupKey = `agent:${agentName}`; const isOpen = expandedDirs.has(groupKey); const tree = buildTree(agentFiles); return <React.Fragment key={agentName}><div onClick={() => toggleDir(groupKey)} style={{ display:"flex", alignItems:"center", gap:6, height:23, paddingLeft:5, cursor:"pointer", color:VS.textMuted, fontSize:10 }}>{isOpen ? <ChevronDown size={11}/> : <ChevronRight size={11}/>}<Icon size={12} color={color}/><span>{agentName}</span><span style={{ marginLeft:"auto", color:VS.textFaint, paddingRight:5 }}>{agentFiles.length}</span></div>{isOpen && <TreeNode name={agentName} node={tree} depth={1} onOpenFile={openFile} activeFilePath={activeTabPath} expandedDirs={expandedDirs} toggleDir={toggleDir}/>}</React.Fragment>; })}</div>
                    <div style={{ flexShrink:0, padding:"10px 10px 12px", borderTop:`1px solid ${VS.border}`, background:"#202020" }}><textarea ref={chatInputRef} value={chatInput} onChange={e => { setChatInput(e.target.value); e.target.style.height="auto"; e.target.style.height=Math.min(e.target.scrollHeight,85)+"px"; }} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (phase !== "building") sendChatMessage(); } }} placeholder="Ask anything, describe an app, or request a change…" rows={2} style={{ width:"100%", boxSizing:"border-box", minHeight:52, maxHeight:85, resize:"none", padding:"9px 10px", border:`1px solid ${VS.borderLight}`, borderRadius:8, background:"#181818", color:VS.textActive, fontFamily:FONT_UI, fontSize:11, lineHeight:1.45, outline:"none" }}/><div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:6 }}><button onClick={() => { setChatInput(""); setTimeout(() => chatInputRef.current?.focus(), 0); }} style={{ border:`1px solid ${VS.border}`, borderRadius:999, background:"transparent", color:VS.textMuted, padding:"3px 8px", fontSize:10, cursor:"pointer" }}>＋ New project</button><button onClick={sendChatMessage} disabled={!chatInput.trim() || phase === "building" || aiThinking} title="Build" style={{ width:28, height:28, border:"none", borderRadius:7, background:chatInput.trim() ? VS.accent : "#38383a", color:chatInput.trim() ? "#fff" : VS.textFaint, display:"flex", alignItems:"center", justifyContent:"center", cursor:chatInput.trim() ? "pointer" : "not-allowed" }}><Send size={13}/></button></div></div>
                  </div>
