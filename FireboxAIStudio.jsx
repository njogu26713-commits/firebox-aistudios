...
export default function FireboxAIStudio() {
  ...
  const startBuild = useCallback(async (desc) => {
    ...
    const structuredEvents = ["agent.started", "task.started", "file.read", "file.created", "file.modified", "file.deleted", "command.started", "command.output", "command.completed", "dependency.installing", "test.started", "test.completed", "preview.starting", "preview.ready", "preview.error", "agent.completed", "agent.failed"];
    const structuredKinds = { "file.read":"files", "file.created":"files", "file.modified":"files", "file.deleted":"files", "command.started":"tool", "command.output":"tool", "command.completed":"tool", "dependency.installing":"tool", "test.started":"tool", "test.completed":"tool", "preview.starting":"preview", "preview.ready":"preview", "preview.error":"preview", "agent.started":"agent", "task.started":"task", "agent.completed":"build", "agent.failed":"error" };
    structuredEvents.forEach(eventName => es.addEventListener(eventName, event => {
      try {
        ...
        appendActivity({ id:`${eventName}-${Date.now()}-${Math.random()}`, kind:structuredKinds[eventName] || "agent", status:isError ? "error" : isDone ? "done" : "working", label:data.title || eventName, text:data.description || detail || "Agent activity received", details:data.details || data.error || "", eventType:eventName });
