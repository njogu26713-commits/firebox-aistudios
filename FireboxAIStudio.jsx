          ...
          {(() => {
            ...
            const editorContent = activity === "home" ? (
              ...
              <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", background:"#181818", color:VS.text, fontFamily:FONT_UI }}>
                ...
                <div style={{ flex:1, minHeight:0, display:"grid", gridTemplateColumns:"1fr", gap:0 }}>
                ...
                <div style={{ flexShrink:0, borderTop:`1px solid ${VS.border}`, background:"#202020", padding:"10px 14px 8px" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:7 }}><div style={{ color:VS.textMuted, fontSize:10, fontWeight:800, letterSpacing:"0.1em" }}>LIVE AGENT ACTIVITY</div><div style={{ color:VS.textFaint, fontSize:10 }}>{phase === "idle" ? "Waiting for a build request" : `${doneCount}/${AGENT_META.length} complete`}</div></div>
                  <div style={{ display:"grid", gridTemplateColumns:isMobile ? "1fr" : "repeat(2, minmax(0, 1fr))", gap:6, maxHeight:isMobile ? 170 : 118, overflowY:"auto" }}>
                    {AGENT_META.map(({ name:agentName, Icon, color }) => { const state = agentStates.find(a => a.name === agentName) || { status:"idle", streaming:"" }; const status = state.status || "idle"; const active = activeAgent === agentName || status === "running" || status === "working"; const label = active ? "Working" : status === "done" ? "Completed" : status === "error" ? "Needs attention" : "Waiting"; const steps = AGENT_STEPS[agentName] || []; const stepIndex = Math.max(0, Math.min((agentVisSteps[agentName] || 1) - 1, steps.length - 1)); const detail = state.streaming || (active || status === "done" ? steps[stepIndex]?.text : "Waiting for the previous agent"); return <div key={agentName} style={{ minWidth:0, display:"flex", alignItems:"flex-start", gap:7, padding:"7px 8px", border:`1px solid ${active ? color + "66" : VS.border}`, borderRadius:7, background:active ? `${color}12` : "rgba(255,255,255,0.02)" }}><Icon size={13} color={color} style={{ marginTop:1, flexShrink:0 }}/><div style={{ minWidth:0, flex:1 }}><div style={{ display:"flex", gap:6, justifyContent:"space-between", color:VS.textActive, fontSize:10, fontWeight:650 }}><span>{agentName}</span><span style={{ color:active ? color : status === "done" ? VS.success : status === "error" ? VS.error : VS.textFaint, fontSize:9 }}>{label}</span></div><div style={{ marginTop:2, color:VS.textMuted, fontSize:9, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{detail || "Ready for work"}</div></div></div>; })}
                  </div>
                </div>
                <div style={{ flexShrink:0, padding:"10px 14px 12px", background:"#202020" }}><div style={{ display:"flex", alignItems:"flex-end", gap:8, maxWidth:980, margin:"0 auto", padding:"8px 9px", border:`1px solid ${VS.borderLight}`, borderRadius:9, background:"#181818" }}><textarea ref={chatInputRef} value={chatInput} onChange={e => { setChatInput(e.target.value); e.target.style.height="auto"; e.target.style.height=Math.min(e.target.scrollHeight,100)+"px"; }} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (phase !== "building") sendChatMessage(); } }} placeholder="Ask anything, describe an app, or request a change…" rows={1} style={{ flex:1, minHeight:30, maxHeight:100, resize:"none", border:"none", outline:"none", background:"transparent", color:VS.textActive, fontFamily:FONT_UI, fontSize:12, lineHeight:1.45 }}/><button onClick={() => { setChatInput(""); setTimeout(() => chatInputRef.current?.focus(), 0); }} style={{ width:30, height:30, border:`1px solid ${VS.border}`, borderRadius:7, background:"transparent", color:VS.textMuted, cursor:"pointer" }} title="New project">＋</button><button onClick={sendChatMessage} disabled={!chatInput.trim() || phase === "building" || aiThinking} title="Build" style={{ width:32, height:32, border:"none", borderRadius:7, background:chatInput.trim() ? VS.accent : "#38383a", color:chatInput.trim() ? "#fff" : VS.textFaint, display:"flex", alignItems:"center", justifyContent:"center", cursor:chatInput.trim() ? "pointer" : "not-allowed" }}><Send size={14}/></button></div></div>
              </div>
            ) : activity === "agents" ? (
              <div style={{ flex:1, overflowY:"auto", background:"#1b1b1c", color:VS.text, fontFamily:FONT_UI }}>
